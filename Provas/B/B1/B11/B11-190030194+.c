...
char moda_vet(unsigned char *vt, int qtd){
        volatile int i, j, cont[2] = {0,0}, moda;
        j =i+1;

        for (i = 0; i < qtd; i++){
                for (j; j < qtd; j++){
                    if (vt[i]==vt[j]) {     //Compara o elemento atual com o proximo
                        cont[0]++;          //Caso igual, incrementa um contador
                        if (cont[0] > cont[1]){ //Se o contador atual for maior do que o passado
                            cont[1] = cont[0];  //Contador passado recebe contador atual
                            moda = vt[i];   //Elemento vigente eh a nova moda
                        }
                    }
                    cont[0] = 0;                //Zera o contador atual
                }
        }
